...
for name in ["dl_sample_pages_1-2.docx", "dl_sample_pages_3-3.docx"]:
    ...
    # check no trailing page-break paragraph or empty para at the end
    tail = []
    for u in units[-3:]:
...
for name in ["dl_sample_pages_1-2.docx", "dl_sample_pages_3-3.docx"]:
    doc = word.Documents.Open(FileName=os.path.join(OUT, name), ReadOnly=True, AddToRecentFiles=False, Visible=False)
    doc.Repaginate()
